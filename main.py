...
class DistancesPresenter:
    ...
    def calculate_distances_between_laser_lines(self,
        ...
        if self.jump_value > len(left_line_right_side_coordinates) or self.jump_value > len(right_line_left_side_coordinates):
            raise line_calculations_errors.JumpValueError(
                f"Jump value ({self.jump_value}) is too big for the length of lines "
                f"(left line length: {len(first_line_right_side_coordinates)},"
                f" right line length: {len(second_line_left_side_coordinates)})."
        ...
        for ((self.yA, self.xA), (self.yB, self.xB)) in zip(first_line_right_side_coordinates,
                                                            second_line_left_side_coordinates):
            z += 1
...
class DataSaver:
    ...
    def save_distances_to_pickle_file(self, distances_list):
        ...
        temp_list = []
        for z in range(self.how_many):
            for i in self.distances_list:
                temp_list.append(i)
        cpickle.dump(temp_list, open('distances.pkl', 'wb'))
        print(temp_list)
